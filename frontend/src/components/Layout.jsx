    ...
    <div className="min-h-screen bg-white dark:bg-slate-900 transition-colors">
      <nav className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 md:px-8 py-4 md:py-5 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div>
            <h1 className="text-lg md:text-2xl font-extrabold text-navy dark:text-blue-400 tracking-tight">셔틀버스 안전</h1>
        ...
        <div className="flex items-center gap-4 md:gap-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-slate-200 dark:bg-slate-700 border-2 border-white dark:border-slate-600 shadow-sm overflow-hidden flex items-center justify-center">
              <span className="material-symbols-outlined text-slate-400 dark:text-slate-500">person</span>
            </div>
          </div>
          <button 
            onClick={logout}
            className="text-slate-500 dark:text-slate-400 hover:text-navy dark:hover:text-blue-400 text-xs md:text-sm font-bold transition-colors border border-slate-200 dark:border-slate-600 px-3 md:px-4 py-2 rounded-full hover:bg-slate-50 dark:hover:bg-slate-700"
          >
            로그아웃
          </button>
        </div>
